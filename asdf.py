...
class Sound():
	def __init__(self, file_path):
		...
		try:
			...
			raise RuntimeError("Error loading audio file")
...
class CNN(torch.nn.Module):
	def __init__(self):
		...
		self.fc = torch.nn.Linear(round(145 / 4) * round(160 / 4) * 64, 3, bias=True)
...
# 미리 학습된 모델 파일 경로
MODEL_PATH = DATA_URI + 'weights/model_state_dict.pt'
...
def main():
	...
	while True: 
		...
		if face_detected:
			labelled_frame = labelled_frame[coords[1]-20:coords[1] + 140, coords[0]-20:coords[0] + 125]
		else:
			labelled_frame = labelled_frame[:160, :145]
